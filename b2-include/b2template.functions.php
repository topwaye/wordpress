...
function get_archives($type='', $limit='') {
	...
	if ('monthly' == $type) {
		++$querycount;
		$arcresults = $wpdb->get_results("SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month` FROM $tableposts WHERE post_date < '$now' AND post_category > 0 AND post_status = 'publish' GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY YEAR(post_date), MONTH(post_date) DESC" . $limit);
		...
		foreach ($arcresults as $arcresult) {
			if ($arcresult->post_date != '1970-01-01 00:00:00') {
