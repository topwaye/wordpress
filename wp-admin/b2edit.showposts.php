...
<form name="viewarc" action="b2edit.php" method="get">
	...
	if ($archive_mode == "monthly") {
		...
		$arc_sql="SELECT YEAR(post_date), MONTH(post_date) FROM $tableposts GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY YEAR(post_date), MONTH(post_date) DESC";
		$querycount++;
		...
		while($row=mysqli_fetch_object($resultarc)) {
			if ($row->post_date != "1970-01-01 00:00:00") {
